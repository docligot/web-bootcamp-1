        ...
        function add_polygon(area, risk_value) {
            ...
            map.addLayer({
                ...
                'paint': {
					...
					map.on('click', layerName, function (region) {
						new mapboxgl.Popup()
						.setLngLat(region.lngLat)
						.setHTML('<h6>' + risk_indicator  + '</h6>' + 
						'<h5>'+'<strong>'+ risk[area][risk_value] + '</strong>'+'</h5>' +
						'<h6>'+ layerName +'</h6>')
						.addTo(map);
						});
					...
					// this should disable hover effect when the mouse leaves but code 
					// is not working
