        ...
        function add_polygon(area, risk_value) {
			...
			var layerNameBorders = ncr.features[area].properties.code;
			var color =[];
			var risk_indicator="";
            ...
            map.addLayer({
				...
				map.on('mouseenter', layerName, function (ncr) {
						// Change the cursor style as a UI indicator.
						map.getCanvas().style.cursor = 'pointer';
						 
						var coordinates = ncr.features[0].geometry.coordinates.slice();
						while (Math.abs(ncr.lngLat.lng - coordinates[0]) > 180) {
						coordinates[0] += ncr.lngLat.lng > coordinates[0] ? 360 : -360;
						}
						 
						popup.setLngLat(ncr.lngLat).setHTML('<h6>' + risk_indicator  + '</h6>' + '<h4>'+'<strong>'+ risk[area][risk_value] + '</strong>'+'</h4>' +
