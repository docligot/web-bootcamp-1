		...
		function load_risk_region() {
			var xhttp = new XMLHttpRequest();
			xhttp.onreadystatechange = function() {
			if (this.readyState == 4 && this.status == 200) {
					risk_region = JSON.parse(this.responseText);
					console.log(risk_region);
				}
			};
			xhttp.open("GET", "extract_risk_region.php", true);
			xhttp.send();
		}
		
		function load_ncr_json() {
			var xhttp = new XMLHttpRequest();
			xhttp.onreadystatechange = function() {
			if (this.readyState == 4 && this.status == 200) {
		...
		function load_risk_ncr() {
			var xhttp = new XMLHttpRequest();
			xhttp.onreadystatechange = function() {
			if (this.readyState == 4 && this.status == 200) {
					risk_ncr = JSON.parse(this.responseText);
					console.log(risk_ncr);
				}
			};
			xhttp.open("GET", "extract_risk_ncr.php", true);
			xhttp.send();
		}
		...
		function set_location(value){
			if(value==1){
				...
				var location = JSON.parse(JSON.stringify(region));
				var risk = JSON.parse(JSON.stringify(risk_region));

			}
			else if(value==2){
				var location = JSON.parse(JSON.stringify(ncr));
				var risk = JSON.parse(JSON.stringify(risk_ncr));
			}
		}
        ...
        function add_polygon(area, risk_value) {
			...
			var layerName = location.features[area].properties.name;
			var layerNameBorders = location.features[area].properties.code;
			var color =[];
			var risk_indicator="";
			// var location = [];
            ...
            map.addSource(sourceName, {
                ...
                'data': {
					'type': 'Feature',
					'geometry': location.features[area].geometry
				}
				// 'generateId': true 
            });
            map.addLayer({
				...
				map.on('mouseenter', layerName, function (region) {
						...
						var coordinates = location.features[0].geometry.coordinates.slice();
						while (Math.abs(location.lngLat.lng - coordinates[0]) > 180) {
						coordinates[0] += location.lngLat.lng > coordinates[0] ? 360 : -360;
						}
						 
						popup.setLngLat(location.lngLat).setHTML('<h6>' + risk_indicator  + '</h6>' + '<h4>'+'<strong>'+ risk[area][risk_value] + '</strong>'+'</h4>' +
