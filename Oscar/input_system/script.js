...
function updateData(product, price, quantity, total) {
	var request = "product=" + product + "&price=" + price +
	"&quantity=" + quantity + "&total=" + total;
	var xmlhttp = new XMLHttpRequest();
	xmlhttp.onreadystatechange = function() {
		if (this.readyState == 4 && this.status == 200) {
			var response = this.responseText;
			document.getElementById('postedData').innerHTML =
			response;
			document.getElementById('statusMessage').innerHTML = 'Data update.';
		} else {
			document.getElementById('statusMessage').
			innerHTML = 'Sending request...';
		}
	}
...
function createTable(period, product, price, quantity, total)
{
	...
	for (var i in period) {
		output += '<tr><td>' + period[i] + '</td><td>' + product[i] + '</td><td>' + price [i] + '</td><td>' + quantity [i] + '</td><td>' + total[i] + '</td><td>';
	}
	output += '</table>'
